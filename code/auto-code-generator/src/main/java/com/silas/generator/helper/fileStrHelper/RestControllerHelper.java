...
import com.silas.util.ColumnUtil;
...
public class RestControllerHelper extends CreateClassHelper{
	...
	public String getOutPutContent() throws Exception {
		...
		someImport.putImport(ImportC.RESTCONTROLLER);
		
		//4.模块
		modules = getModules();
		String methods = ModuleUtils.getModulesStr(modules);
		modules.add(someImport);
		importNeed = getImortStr(modules);
		//5。结束
		String end = "}";
		//6。拼接
...
//		}
		...
		public ModuleMethod fields(){
			...
			method.setFullStr(fullStr);
			//要导入的包
			method.putImport(packagePath+".service."+entityName+"Service");//服务层包
		...
		public ModuleMethod save(){
			...
			String s = n+
				...
				if(ColumnUtil.hasColumn(colList, "status")) {
					s +="				record.setStatus(1);//默认1为未删除,0则已删除\r\n"; 
				}
				if(ColumnUtil.hasColumn(colList, "create_time")) {
					s +="				record.setCreateTime(new Date());//创建时间\r\n";
				}
				s +="				suc = "+entityLowerName+"Service.save(record);\r\n"+
					"			}else {//更新操作\r\n" + 
